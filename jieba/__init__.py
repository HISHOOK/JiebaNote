...
re_han_default = re.compile("(.+)", re.U)  # 此处做了更改，可以支持空格和特殊字符
...
class Tokenizer(object):
    ...
    def gen_pfdict(self, f):
        ...
        for lineno, line in enumerate(f, 1):  # enumerate(_, start_index)
            try:
    ...
    def initialize(self, dictionary=None):
        ...
        with self.lock:
            ...
            try:
                ...
                # modify_time = os.stat(cache_file).st_mtime
                # now_time = time.time()
                # diff_sec = now_time - modify_time
    ...
    def calc(self, sentence, DAG, route):
        ...
        # 从后往前计算，并初始化末尾为0
        route[N] = (0, 0)
        logtotal = log(self.total)  # 取log防止下溢
    ...
    def get_DAG(self, sentence):
        '''
        构建有向无环图：
        最终的DAG是以{k:[k, j, ...], m:[m, p, q], ...}的字典结构存储，
        其中k和m为词在文本sentence中的位置，k对应的列表存放的是文本中以k开始且词sentence[k: j + 1]在前缀词典中的 以k开始j结尾的词的列表，
        即列表存放的是sentence中以k开始的可能的词语的结束位置。
        '''
        self.check_initialized()  # 检查是否初始化（初始化中进行了构建前缀词典）
        # 下面构建有向无环图
        DAG = {}
        N = len(sentence)
        for k in xrange(N):
